...
def get_loaders_eval(dataset, args):
    ...
    if dataset == 'cifar10':
        ...
        valid_data = dset.CIFAR10(
            root=args.data_dir, train=False, download=True, transform=valid_transform)
    ...
    train_queue = torch.utils.data.DataLoader(
        ...
        sampler=train_sampler, pin_memory=True, num_workers=args.workers)

    valid_queue = torch.utils.data.DataLoader(
        valid_data, batch_size=args.batch_size, shuffle=False,
        sampler=valid_sampler, pin_memory=True, num_workers=args.workers)
