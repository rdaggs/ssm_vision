...
class DownsampleWithAttention(nn.Module):
    ...
    Down-sampling block with attention-based pooling.
    ...
    def __init__(self, dim, keep_dim=False):
        ...
        self.conv = nn.Conv2d(dim, dim_out, 3, 2, 1, bias=False)  # Standard downsampling conv

        self.attention_pool = nn.Sequential(

            nn.Conv2d(dim, 1, kernel_size=1),  # Generate attention weights
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            Tensor with downsampled features.
        ...
        attention_weights = self.attention_pool(x)  # [B, 1, H, W]

        attention_weights = attention_weights.view(B, 1, -1)  # Flatten spatial dimensions

        x = x.view(B, C, -1)  # Flatten spatial dimensions

        x_attn = torch.bmm(x, attention_weights.permute(0, 2, 1))  # Weighted aggregation

        x_attn = x_attn.view(B, C, H // 2, W // 2)  # Reshape back to spatial dimensions



        # Combine convolutional and attention outputs (optional)

        x_combined = x_conv + x_attn  # Combine features
...
class MambaVisionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.downsample = None if not downsample else DownsampleWithAttention(dim=dim)
        self.do_gt = False
